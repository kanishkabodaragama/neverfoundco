import Image from "next/image";

export function CartHero() {
  return (
    <section className="grid gap-6 bg-[#123f32] px-5 py-10 text-[#ead8bd] md:min-h-[610px] md:grid-cols-[0.9fr_1.1fr] md:px-8 lg:px-10 xl:px-12">
      <div className="relative flex min-h-[480px] flex-col justify-center overflow-hidden py-8 md:min-h-[610px]">
        <h1 className="font-black uppercase leading-[0.78] tracking-[-0.07em] text-[clamp(4rem,8.5vw,9rem)]">
          Your Cart
        </h1>
        <p className="font-hand mt-6 max-w-lg text-2xl uppercase leading-tight tracking-wide text-[#d9532f] md:text-3xl">
          Good choices. No rush.
          <br />
          Check it. Love it. Get it.
        </p>
        <p className="mt-8 w-fit rotate-[-2deg] bg-[#d8bf8f] px-6 py-4 text-center font-hand text-2xl uppercase leading-tight text-[#17251f]">
          Don&apos;t sleep
          <br />
          on it.
        </p>
        <div className="absolute right-8 top-8 hidden text-6xl text-[#efc067] md:block">
          ✶
        </div>
        <div className="absolute right-16 top-[50%] hidden rounded-full bg-[#d9532f] px-4 py-4 text-4xl md:block">
          ☠
        </div>
        <div className="absolute bottom-2 left-0 h-3 w-[78%] bg-[repeating-linear-gradient(90deg,#ead8bd_0_12px,#123f32_12px_24px)]" />
      </div>
      <div className="relative min-h-[480px] overflow-hidden md:min-h-[610px]">
        <div className="landing-noise pointer-events-none absolute inset-0 z-20" />
        <figure className="paper-frame absolute left-[8%] top-8 h-[72%] w-[46%] rotate-[-2deg] overflow-hidden bg-[#2f8088]">
          <Image
            alt="Skateboarder wearing Never Found clothing in a bowl"
            className="object-cover"
            fill
            priority
            src="/images/landing/hero-skate.svg"
          />
        </figure>
        <figure className="paper-frame absolute right-[5%] top-6 h-[76%] w-[42%] rotate-[2deg] overflow-hidden bg-[#ead8bd]">
          <Image
            alt="Never Found shirt lifestyle photo with city skyline"
            className="object-cover"
            fill
            src="/images/landing/lookbook-3.svg"
          />
        </figure>
        <div className="absolute bottom-20 right-14 z-30 rounded-full border-[3px] border-[#ead8bd] px-4 py-4 text-5xl">
          🌐
        </div>
        <div className="absolute bottom-8 right-8 z-30 text-3xl font-black uppercase leading-none text-[#d9532f]">
          World
          <br />
          Wide
        </div>
      </div>
    </section>
  );
}
