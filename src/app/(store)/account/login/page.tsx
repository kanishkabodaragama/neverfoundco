      ...
      <div className="mx-auto grid min-h-[70vh] w-full max-w-xl place-items-center px-5 py-12">
        <form action="/api/account/login" className="w-full border border-ink bg-transparent p-6" method="post">
          ...
          <div className="mt-6 grid gap-4">
            <label className="grid gap-2 font-sans text-xs font-bold uppercase tracking-normal">
              Email
          ...
          <button className="mt-6 bg-ink px-5 py-3 font-sans text-xs font-bold uppercase tracking-normal text-acid transition-colors hover:bg-rust hover:text-ink" type="submit">
            Login
          </button>
          {error ? <p className="mt-4 font-sans text-xs font-bold uppercase tracking-normal text-rust">{error}</p> : null}
        </form>
      </div>
      </StoreArtSurface>
      <Footer />
