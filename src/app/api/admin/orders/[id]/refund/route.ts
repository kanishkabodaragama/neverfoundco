...
import { refundStatusSchema } from "@/lib/validation/admin";
...
export async function PATCH(
  ...
  const parsed = refundStatusSchema.safeParse(body);

  if (!parsed.success) {
    return adminRedirect(request, `/admin/orders/${id}`, {
      error: parsed.error.issues[0]?.message ?? "Invalid refund status.",
    });
  }
